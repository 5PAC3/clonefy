...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $anno = !empty($_POST['anno']) && trim($_POST['anno']) !== '' ? trim($_POST['anno']) : null;
  $user_id = $_SESSION['id'];
    ...
    if (empty($errori)) {
      ...
      if (move_uploaded_file($audio['tmp_name'], $percorso_completo)) {
        ...
        if ($anno !== null && is_numeric($anno)) {
          ...
          $stmt->bind_param("sssisi", $titolo, $artista, $genere, $anno, $percorso_completo, $user_id);
        } else {
          $stmt = $conn->prepare("INSERT INTO songs (titolo, artista, genere, file_path, user_id) VALUES (?, ?, ?, ?, ?)");
          $stmt->bind_param("ssssi", $titolo, $artista, $genere, $percorso_completo, $user_id);
        }
        if ($stmt->execute()) {
